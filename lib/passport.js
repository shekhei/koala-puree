...
class EskygoStrategy extends strategy {
	...
	authenticate(req, options) {
		var self = this;
		this._verify(options.username, "eskygo", this._app._crypter.hash(options.password), function verified(err, user, info){
			...
			yield (new Promise(function(resolve, reject){
				...
				passport.use(
					new EskygoStrategy(function(alias, provider, password, done) {
						...
						self.services('eskygo-user-service').post('/users/check', {alias:alias, provider: provider, password: password}, {}).then(function(res){
                            ...
                            if ( true !== body.res ) {
                                return done(new Error("User is not correct"),null, body);
                            }
							self.services('eskygo-user-service').get(`/users/${body.userId}`,{},{}).then(function(res){
                                if ( 200 !== res.status ) {
                                    return done(new Error("User is not found"), null, res.body);
                                }
						...
						if ( obj.user && obj.user.id && obj.user.alias ) {
							...
							Promise.all([
								...
								self.services('eskygo-user-service').get(`/users/${obj.user.id}/providers/${obj.user.provider}`)
									...
									function getRoles(resolve, reject){
										if ( user._roles ) {
											return resolve(user._roles);
										}
										debug("Roles is not retrieved for this user yet, retrieving now");
										self.services('eskygo-user-service').get(`/users/${user.id}/roles`)
											.then(function(userRoles){
												...
												user._roles = userRoles.sort();
												resolve(user._roles);
											});
									}
									user.roles = new Promise(getRoles);
									done(null, user);
								})
						} else {
				...
				self.ensureRole = function(roles, success, fail) {
					...
					return function*(next){
						...
						var userRoles = yield user.roles;
						if ( roles.length > userRoles ) {
