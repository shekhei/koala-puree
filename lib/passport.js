			...
			yield (new Promise(function(resolve, reject){
				...
				passport.use(
					new EskygoStrategy(function(alias, provider, password, done) {
						...
						puree.services('eskygo-user-service').post('/user/check', {alias:alias, provider: provider, password: password}, {}).then(function(res){
							var body = JSON.parse(res.body);
							puree.services('eskygo-user-service').get(`/user/${body.userId}`,{},{}).then(function(res){
				...
				passport.deserializeUser(function(key, done){
					JWT.verify(key).then(function(obj){
						if ( obj.user && obj.user.id ) {
							Promise.all([
								puree.services('eskygo-user-service').get(`/user/${obj.user.id}`,{},{}),
								puree.services('eskygo-user-service').get(`/user/${obj.user.id}/providers/${obj.user.provider}`)
