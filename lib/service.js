...
Service.prototype = {
		    ...
		    return new Promise(function(resolve, reject){
			    ...
			    if ( self.sioClient ) {
				    ...
				    if ( serviceConfig === undefined ) {
					    try {
						    serviceConfig = readYaml.sync(require("path").resolve(self._app._basePath,"./config/services.yml"))[self._app._app.env];
					} catch(e) {
						    debug(e.stack);
						    serviceConfig = false;
					}
    ...
    let sc = (self.sioClient = self.sioClient || new require("socket.io-client")(`ws://${uri.host}${uri.path}`, {transports:["websocket"]}));
