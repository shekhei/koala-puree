		...
		return new Promise(function(resolve, reject){
			...
			self._retrieve().then(function(service){
				if ( service.replyDomain === "local.") { // this is a local service
					service.addresses[0] = "127.0.0.1"
				}
				debug(`service(${self._name}@${self._ver}) resolved, connecting now@ws://[${service.addresses[0]}]:${service.port}`);
				let sc = (self.sioClient = self.sioClient || new require('socket.io-client')(`ws://[${service.addresses[0]}]:${service.port}`));
				sc.emit('s', method, path, data, headers,function(status, headers, body){
