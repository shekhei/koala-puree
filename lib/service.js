...
Service.prototype = {
	...
	request: function(method, path, data, headers) {
		...
		function _request(sc, path, nsp, method, data, headers, resolve, reject) {
			...
			realPath+=path;
			debug("making the request to", realPath);
		...
		return new Promise(function(resolve, reject){
			...
			if ( self.sioClient ) {
				...
				if ( serviceConfig === undefined ) {
					try {
						serviceConfig = readYaml.sync(require('path').resolve(self._basePath,"./config/services.yml"))[process.env.NODE_ENV.toLowerCase()];
					} catch(e) {
						serviceConfig = false;
					}
				}
