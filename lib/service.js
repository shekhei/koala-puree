	...
	request: function(method, path, data, headers) {
		...
		return new Promise(function(resolve, reject){
			...
			self._retrieve().then(function(service){
				debug(`service(${self._name}@${self._ver}) resolved, calling its service now`);

				resolve(new Promise(function(resolve, reject){
					let sc = (self.sioClient = self.sioClient || new require('socket.io-client')(`ws://${service.addresses[1]}:${service.port}`));
